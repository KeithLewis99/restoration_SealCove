...
for (i in 1:length(df_all$Station)){
  ...
  if (df_all$Station[i] == "C3" |
  ...
  } else if (df_all$Station[i] == "T3" |
             df_all$Station[i] == "3"){
    df_all$Station_new[i] <- "C3"
  } else if (df_all$Station[i] == "T4" |
             df_all$Station[i] == "4"){
    df_all$Station_new[i] <- "C4"
  } else if (df_all$Station[i] == "T5" |
             df_all$Station[i] == "5"){
    df_all$Station_new[i] <- "C5"
  } else if (df_all$Station[i] == "T6" |
             df_all$Station[i] == "6"){
    df_all$Station_new[i] <- "C6"
  } else if (df_all$Station[i] == "T7" |
             df_all$Station[i] == "7"){
    df_all$Station_new[i] <- "C7"
  } else if (df_all$Station[i] == "T8" |
             df_all$Station[i] == "8"){
    df_all$Station_new[i] <- "C8"
  } else if (df_all$Station[i] == "T9" |
             df_all$Station[i] == "9"){
    df_all$Station_new[i] <- "C9"
    ...
    df_all$Station_new[i] <- "D2"
             ...
             df_all$Station[i] == "T1" & df_all$Year[i] >= 1990){
    df_all$Station_new[i] <- "C1"
  } else if (df_all$Station[i] == "2" & df_all$Year[i] >= 2015 |
             df_all$Station[i] == "T2" & df_all$Year[i] >= 1990){
    df_all$Station_new[i] <- "C2"
...
df_sum <- df_all |>
  ...
  filter(!(Year == 1993 & (Station_new == "C4" | Station_new == "C5"))) |>
...
# edit grid
# don't need to remove structural zeros for Seal Coveremove the structural zeros, i.e., sites that weren't fished
...
df_grid1 <- df_grid |>
  filter(!((Year > 1989) & 
           ...
           (Station_new == "C15" | Station_new == "C16" |
              Station_new == "C1" | Station_new == "C2" |
              Station_new == "C3" | Station_new == "C4" |
              Station_new == "C5" | Station_new == "C6" |
              Station_new == "C7" | Station_new == "C8" |
              Station_new == "C9")) &
           !(Year <= 1991 & Station_new == "C10L") &
           !((Year == 1993) & (Station_new == "C4" | Station_new == "C5"))
  )
...
# add a C
df_area$Station_new <- ifelse(grepl("^[0-9]+$", df_area$Station_new), paste0("C", df_area$Station_new), df_area$Station_new)
unique(df_area$Station_new)

# add 1993 Stn 8 ASY and 1992 Stn 7 AS
tmp <- df_area[1:2,]
tmp[, 1:17] <- NA
tmp[1, c(1, 2, 16, 17)] <- c("AS", 1992, 58, "C7")
tmp[2, c(1, 2, 16, 17)] <- c("ASYOY", 1993, 72, "C8")
tmp[, c(1, 2, 16, 17)]
...
df_area <- df_area |>
  filter(
    !(Species == "AS" & Year == 1992 & Station_new == "C7") |
      (Species == "ASYOY" & Year == 1993 & Station_new == "C8"))

df_area <- rbind(df_area, tmp)
...
df_a1 <- left_join(df_a, df_area[, c(1, 2, 16, 17)], 
          by = c("Year", 
                 "Species",
                 "Station_new"
                   ))
...
df_area |> filter(
  (Year == 1992 & Station_new == "C7") |
    (Year == 1993 & Station_new == "C8")) |> select(Year, Station_new, Species, Area, Density_100)

df_a |> filter(
  (Year == 1992 & Station_new == "C7") |
    (Year == 1993 & Station_new == "C8")
