...
# To do
- Fig 5 (trend plots) & 7 (large fish plots) where was this done - need the file: haven't received these.
...
Other issues (Dave Cote):    
    ...
    - Independence issues - see Zuur and Dave.  
        - Dave showed me how to look at Temporal Independence and Spatial independence using package DHARMa (https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html#workflow-in-dharma).  We did Temproal by year and there are no issues.  Spatial will require lat/long converted to UTM.  However, i'm not sure how the "Stations" in BT_estimates_by_site correspond to those on the map (see below).  See glmm_biomass_models.R, glmm_density_models.R, and glmm_anova.R.  
...
## Linear Model
Basically, GLM can't handle random effects (for repeated measures) and LMM can't handle non-Gaussian distributions.  So this leaves GLMM.
...
Therefore, conclude that reaonable inferences are being made and get on with it!!!!  
- A final problem was that in the orginal projection, the eastings are stretched out and it distorted the spatial relationships among the Stations.  I fixed this by modifying the xlim.  
...
For BT, there are more fish and higher biomass in pools than riffles.  There is a general increase in the pools but this is driven to some degree by 2016 - suggests more fish.  The riffle controls are oscilate without trend upstream and downstream while density is domeshaped downstream but increasing upstream suggesting bigger fish.  
...
AS may have a few more fish in the riffles but biomass is higher in the pools.  Density and biomass oscilate without trend in the pools but density increases in the treatment riffles while biomass levels off.  Both biomass and density seem to increase in teh controls.
...
Lunkers appear to be good for BT but for AS, there is no effect.  For YOY, pool C3 is good but the rest are no.  Again, predation may be a factor.  
