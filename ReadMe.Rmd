...
https://stats.stackexchange.com/questions/235199/how-to-build-a-generalized-linear-mixed-model-with-repeated-measures-in-rc
https://tutorials.inbo.be/tutorials/r_fixed_random/

And Isabelle Ghement
https://stats.stackexchange.com/questions/398444/how-to-correctly-model-repeated-measures-random-effects-in-a-linear-mixed-effect
https://www.researchgate.net/post/For_generalized_mixed_effect_models_is_it_possible_to_appoint_a_variable_as_fixed_effect_and_random_effect_simultaneously
- This shows how to visualize in simple way
https://tutorials.inbo.be/tutorials/r_fixed_random/  

** Conclude ** Stick with it and deal if the reviewer gets upset.  
...
# re-think presentation of results
Upon re-reading the ms, after it had been out for review from June 2024 to December, i found that the INtro and Methods were really good.  However, I was a bit disappointed with the Results - I found them very hard to read which is bad news for any potential reviewer.  I don't want to rely on p-values.  We are interesed in effect sizes and the thing is how to present them. I re-read all my "problem with p-values" papers and found that there are lots of was to present effect sizes (see Nakagawa and Cuthill 2007) but in this case, the parameter estimates are telling us what we want to know, i.e., the differences between the control-after v. control before, impact-after v. impact-before, and impact-after v. impact-before.

According to ChatGPT (I could find nothing on Google, you just have to take the exponential of the parameter estimate to back-transform it - see scratch_pad_variance.R)

But I get a curious result ito the models with numYear.  What happens is that the intercept goes to zero and then you need the year (fixed and random) to get to a value that makes sense.  This value is then modified by the parameter estimates for the rest of the fixed terms (treat*time).  I replicated this using code in scratch_pad_variance.R and get the same result even when using Tweedie distribution.  The problem that I had is that the fitted values are probably not all that close to the actual values, i.e., these models aren't great.    But proceed with reporting the CIs for the parameter estimates.

# A note on natural logarithms and presenting results
So, natural logarithms are an approximation of percent change when the values are small.  There are times when its preferable to use logs on their own over percent change but I think that we biologist types prefer to use percent change as its more intuitive.  So I think that, while the logarithmic approximation is useful and good to know, i'll do the conversions to show the actual change when the parameter estimate doesn't overlap 0.  

Also, i'm pretty sure that the parameter estimates, which are the change from the reference (Intercept = Control:After) is an approximation of this percent change so that gives some rough guidance.

These are a few handy web pages:  
...
https://stats.stackexchange.com/questions/521180/percentage-change-vs-log-change-in-a-repeated-measures-or-time-series-analysis
https://people.duke.edu/~rnau/411log.htm
https://stats.stackexchange.com/questions/488691/why-can-i-interpret-a-log-transformed-dependent-variable-in-terms-of-percent-cha

See below for an example using BT Density in non-pools - Intercept + Time
```{r, echo=TRUE}
# Intercept
exp(2.548)
# Intercept + Time
exp(2.548 + 0.370)
# Percent change with Back-transformed values
(exp(2.548 + 0.370) - exp(2.548))/exp(2.548 + 0.370)
# values are 0.37 ~ 0.38  and for percent change: 0.37*100 ~ 0.38*100 - I don't think that the denominator is right in this case - see next section
```

# A note on percent change
So, I think in general, if you are doing percent change, its just (new-old)/old*100.  For a linear model on natural scale, you can just do: 
$$ [(Int + \beta) - Int]/Int*100$$
Which reduces to:
$$\beta/Intercept*100$$

But when its on the log scale:
$$(e^{Int + \beta} - e^{Int})/e^{Int}$$
which reduces to:
$$(e^{\beta} - 1)*100$$
Note that much of the above is from ChatGPT

# a note on percent change an zi

So, the above is right but when you have zi models, what's the impact on percent chane.  My understanding, based on my own work and work with ChatGPT, is that when ziformula ~ 1, there is no impact.  But there will be when ziformula ~ variables.  

See the percent_change_chatgpt.qmd for more plus notes (2025-06-18)

# More on percent change (2025 - early Sept)
I have spent the last week trying to better understand how glmmTMB plays with emmeans in order to extract comparisons and then calculate percent change while taking the hurdle model into account. especially when the ziformula = !~ 1.  See scratch_pad_emmeans where I went through the exercise for the density models for AS, ASYOY, and BTYOY - I did not do BT beause there are no zeros.  I figured out how the emmeans function works and then calculated percent change including when ziformula = ~ Time.  I compared these multiple ways.
Note that I did not figure out why the options "cond" and "cmean" for the component arguement differ sometime but its time to move on.
...
# Notes  
- I tried to find out how many times LUNKERs have been installed in NL but incredibly, it seems that we do not have this information.  Perhaps the Province??